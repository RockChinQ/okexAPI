...
def updateCurrent():
    ...
    # profit
    profit = VirtualTrading.fund + float(
        VirtualTrading.product) * CandleMgr.currentCandle.close - VirtualTrading.initialFund
    ...
    canvas.create_text(60, 115, text="(%.5f %%)" % (profit / VirtualTrading.initialFund * 100), font=("Serif", 13),
                       fill=fillcl)
    ...
    if VirtualTrading.affordable():
        ...
        canvas.create_rectangle((tempCurrentX % rowCount) * rowWidth + dx + 3, highRY,
                                (tempCurrentX % rowCount) * rowWidth + dx + rowWidth - 3, highRY + 1, fill="green",
                                outline="green")
    if VirtualTrading.sellable():
        ...
        canvas.create_rectangle((tempCurrentX % rowCount) * rowWidth + dx + 3, lowRY,
                                (tempCurrentX % rowCount) * rowWidth + dx + rowWidth - 3, lowRY + 1, fill="red",
                                outline="red")
...
def updateStatus():
    ...
    canvas.create_text(60, 170, text="(%.5f %%)" % (VirtualTrading.burnt() / VirtualTrading.initialFund * 100),
                       font=("Serif", 13),
                       fill="red")
    ...
    # this trade
    if VirtualTrading.sellable():
        canvas.create_text(50, 370, text="buy at", fill="white")
        fillcl="red"
...
def resetCanvas():
    ...
    canvas.create_line(0, 501, 800, 501, fill="white")
    # KGraph border
    canvas.create_line(rowCount * rowWidth + dx + 1, 0, rowCount * rowWidth + dx + 1, top.winfo_height(), fill="white")
    for candle in CandleMgr.candles:
        if not int(index / rowCount) == pageNow:
            ...
            c = canvas.create_rectangle((index % rowCount) * rowWidth + rowWidth / 2 - 4 + dx, sarY-1,
                                        (index % rowCount) * rowWidth + rowWidth / 2 + 4 + dx, sarY + 3,
                                        fill="yellow")
        index = index + 1
    canvas.pack()
